...
def test_compile_workflow_with_judge_nodes():
    """Test compiling a workflow with judge nodes."""
    spec = Spec(
        version="0.1",
        llms={"llm1": LLM(**VALID_LLM_CONFIG)},
        workflow=Workflow(
            ...
            nodes=[
                WorkflowNode(id="start", kind="agent", ref="llm1"),
                WorkflowNode(id="judge1", kind="judge", ref="llm1"),
                WorkflowNode(id="end", kind="agent", ref="llm1", stop=True)
            ],
            edges=[
                Edge(source="start", target="judge1"),
                Edge(source="judge1", target="end")
            ]
        )
    )

    # Verify that compilation succeeds for workflows with judge nodes
    graph = compile_to_langgraph(spec)
    assert graph is not None
